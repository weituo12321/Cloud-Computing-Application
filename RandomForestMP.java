...
public final class RandomForestMP {
    ...
    private static class TestDataToPoint implements Function<String, Vector> {
        private static final Pattern TSPACE = Pattern.compile(",");
        public Vector call(String line) throws Exception {
            String[] tok = TSPACE.split(line);
            double[] point = new double[tok.length - 1];
            for (int i = 0; i < tok.length - 1; ++i) {
                point[i] = Double.parseDouble(tok[i]);
            }
            Vector re = Vectors.dense(point);
            return re;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        //JavaRDD<LabeledPoint> testingData = MLUtils.loadLibSVMFile(sc.sc(), test_data_path).toJavaRDD(); 
        ...
        model = RandomForest.trainClassifier(trainingData, numClasses, categoricalFeaturesInfo,numTrees, featureSubsetStrategy,impurity, maxDepth, maxBins, seed);



        JavaRDD<LabeledPoint> results = test.map(new Function<LabeledPoint, LabeledPoint>() {
            public LabeledPoint call(LabeledPoint points) {
                Vector featureVec = points.features();
                return new LabeledPoint(model.predict(featureVec), featureVec);
            }
        });
